...
创建**无限的用户和模型等级**，具有不同的权限级别：

- **用户等级 (0-∞)**：定义免费用户、付费用户、高级用户等
- **模型等级 (0-∞)**：按成本/能力对模型分类
- **灵活匹配**：严格等级匹配或自定义白名单/黑名单逻辑

*示例*：Tier 0 用户获得 10 RPM，Tier 1 用户获得 100 RPM，Tier 2 用户无限制。
...
3. **完成！** 所有用户现在限制为 10 请求/分钟，100/小时。
...
| 字段 | 描述 | 示例 |
|-------|-------------|---------|
| `tier_id` | 唯一等级标识符 | `0`, `1`, `2` |
| `tier_name` | 显示名称 | `"免费"`, `"高级"` |
| `emails` | 此等级中的用户 | `["user@example.com"]` |
| `rpm` | 每分钟请求数 | `10` |
| `rph` | 每小时请求数 | `100` |
| `win_time` | 滑动窗口（分钟） | `1440` (24小时) |
| `win_limit` | 窗口内请求数 | `500` |
| `clip` | 上下文消息限制 | `10` |
